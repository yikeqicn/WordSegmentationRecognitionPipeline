...
def segmentation_wraper(img_path,word=False): # yike: should revise a little if the input is already cv2 images
  ...
  if word:
    images=[cv2.transpose(cv2.resize(img, (args.imgsize[0],args.imgsize[1]), interpolation=cv2.INTER_CUBIC))]
...
#  print(len(list(detect_rlt)))
  ...
  for batch in batchnize(iterable=detect_rlt,size=args.bounding_box_batch_size):
   ...
   # print(org_img_paths)
    ct=ct+1
    line_image_lists= list(map(lambda path: segmentation_wraper(path,word=True),org_img_paths)) # assume segmentation works, risky
